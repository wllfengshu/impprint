 			...
 			<div class="col-md-2" style="margin-top: 30px;">
			 	<div class="panel panel-default">
				  ...
				  <ul class="list-group">
				    ...
				    <li class="list-group-item active"><a href="sendfile" style="color: black;">待送货订单</a></li>
				 	 ...
				 	 <div class="panel-body" style="min-height: 500px;">
				 	 	<table class="table table-hover table-bordered">
							...
							<c:if test="${empty requestScope.pageorders }">
								<tr><td></td><td>暂无待送货订单</td><td></td><td></td><td></td><td></td></tr>
							</c:if>
							<%
								...
								%>
									...
									<td >
										...
										<%=f.getOrdermoney() %>元<br>
										<a title="<%=f.getMoreword() %>" style="color: black;"><%=f.getMoreword() %></a>
										
									</td>
									<td >
										<a _href="changeorder?ordernum=<%=ordernum %>&finish=3" title="确定后提交到结束订单列表" class="btn btn-info btn-xs btn-danger btn-finish">送达</a>
						  ...
						  <ul class="pagination">
						    ...
						    <c:if test="${requestScope.pagination.page > 1 }">
						    	 <a href="sendfile?page=${requestScope.pagination.page-1 }" aria-label="Previous">
							        <span aria-hidden="true">&laquo;</span>
							      </a>
						    </c:if>
						    <c:if test="${requestScope.pagination.page == 1 }">
						    	 <a href="sendfile?page=1" aria-label="Previous">
							        <span aria-hidden="true">&laquo;</span>
							      </a>
						    </c:if> 
						    </li>
						   ...
						   Pagination p = (Pagination)request.getAttribute("pagination");
						   		...
						   		%>
						   		 <li <% if(p.getPage() == i){%>class="active"<% } %>><a href="sendfile?page=<%=i%>"><%=i %></a></li>
						   		<% 
						   		}
						    %>
						    ...
						    <li>
						    	<c:if test="${requestScope.pagination.page < requestScope.pagination.total}">
						    	 <a href="sendfile?page=${requestScope.pagination.page-1 }" aria-label="Next">
								        <span aria-hidden="true">&raquo;</span>
								      </a>
							    </c:if>
							    <c:if test="${requestScope.pagination.page >= requestScope.pagination.total}">
							    	 <a href="sendfile?page=${requestScope.pagination.total }" aria-label="Next">
								        <span aria-hidden="true">&raquo;</span>
								      </a>
							    </c:if> 

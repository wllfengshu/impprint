...
public class SendFileServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			pstmt = con.prepareStatement("select distinct ordernum from paths where finish=2 order by date desc");
			...
			//request.setAttribute("sendfiles", files);
			request.setAttribute("pagination", pagination);
			
			request.getRequestDispatcher("admin/send.jsp").forward(request, response);
